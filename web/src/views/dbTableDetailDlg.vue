<template>
  <el-dialog v-model="showTableDetailDlg" :title="title" width="1000px">
...
export default {
  ...
  methods: {
    showTableDetail(table) {
      ...
      this.getDbColumn(table.tableName);
      this.getDbIndex(table.tableName);
      this.showTableDetailDlg = true;
    },

    async getDbColumn(tableName) {
      const { code, data, msg } = await dbManager.getDbColumn({'tableName' : tableName});
      if ('200' == code) {
        if (data && data.length > 0) {
          for (let i = 0; i < data.length; i++) {
            data[i].isPrimaryStr = data[i].isPrimary == 1 ? '是' : '';
            data[i].isNullableStr = data[i].isNullable == 1 ? '' : '不为空';
    ...
    async getDbIndex(tableName) {
      const { code, data, msg } = await dbManager.getDbIndex({'tableName' : tableName});
      if ('200' == code) {
        if (data && data.length > 0) {
          for (let i = 0; i < data.length; i++) {
            data[i].isUniqueStr = data[i].isUnique == 1 ? '是' : '';
          }
        }
        this.indexData = data;
      }
