...
/**
	删除表，以备份方式删除表
		V_TABLE_NAME 表名
*/
CREATE PROCEDURE PR_DEL_TABLE(IN V_TABLE_NAME VARCHAR(40))
BEGIN
	-- 声名变量
 	DECLARE V_COUNT INT;
    ...
    -- 如果备份表已存在则删除备份表
    SET @V_SQL = CONCAT(" DROP TABLE IF EXISTS ", V_BAK_TABLE_NAME);
	...
	IF V_COUNT = 1 THEN
		-- 存在则备份表
		SET @V_SQL = CONCAT(" ALTER TABLE ", V_TABLE_NAME, " RENAME TO ", V_BAK_TABLE_NAME);
        PREPARE sqlStmt FROM @V_SQL;
		EXECUTE sqlStmt;
    END IF;
END;
