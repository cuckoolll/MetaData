...
BEGIN
	...
	IF V_COUNT = 1 THEN
		-- 查询字段列是否存在
		SELECT COUNT(1) INTO V_COUNT FROM INFORMATION_SCHEMA.COLUMNS  WHERE TABLE_NAME = V_TABLE_NAME AND TABLE_SCHEMA = V_TABLE_SCHEMA AND COLUMN_NAME = V_COL_NAME;
		-- 字段不存在则新增字段
		IF V_COUNT = 0 THEN
			SET @V_SQL = CONCAT(" ALTER TABLE ", V_TABLE_SCHEMA, ".", V_TABLE_NAME, " ADD COLUMN ", V_COL_NAME, " ", V_COL_DATA_TYPE, V_COL_FINAL_SCALE, " ", V_COL_DEFAULT_SQL, " COMMENT '", V_COL_REMARK, "'");
		...
		-- 字段存在，修改字段
			IF V_COL_DATA_TYPE = 'decimal' THEN
				...
				SELECT numeric_scale INTO V_OLD_COL_ACCURACY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = V_TABLE_NAME AND TABLE_SCHEMA = V_TABLE_SCHEMA AND COLUMN_NAME = V_COL_NAME;
				SET @V_SQL = CONCAT(" SELECT MAX(LENGTH(", V_COL_NAME, ")) INTO @V_MAX_INT_LENGTH FROM ", V_TABLE_SCHEMA, ".", V_TABLE_NAME);
				PREPARE sqlStmt FROM @V_SQL;
				EXECUTE sqlStmt;
				DEALLOCATE PREPARE sqlStmt;
				SET V_MAX_INT_LENGTH = @V_MAX_INT_LENGTH - V_OLD_COL_ACCURACY - 1;
				IF V_COL_LENGTH - V_COL_ACCURACY < V_MAX_INT_LENGTH THEN
					SELECT CONCAT("表", V_TABLE_SCHEMA, ".", V_TABLE_NAME, "，列", V_COL_NAME, "类型为", V_OLD_COL_TYPE, "表中最大整数长度为", V_MAX_INT_LENGTH, "，长度不足") ERR;
				ELSE
					SET @V_SQL = CONCAT(" ALTER TABLE ", V_TABLE_SCHEMA, ".", V_TABLE_NAME, " MODIFY COLUMN ", V_COL_NAME, " ", V_COL_DATA_TYPE, V_COL_FINAL_SCALE, " ", V_COL_DEFAULT_SQL, " COMMENT '", V_COL_REMARK, "'");
					PREPARE sqlStmt FROM @V_SQL;
					EXECUTE sqlStmt;
				END IF;
			ELSE 			
