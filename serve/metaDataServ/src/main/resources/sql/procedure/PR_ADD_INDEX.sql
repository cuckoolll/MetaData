...
BEGIN
	...
	IF V_COUNT = 1 THEN
        ...
        IF V_COUNT > 0 THEN
			SELECT CONCAT(V_TABLE_SCHEMA, ".", V_TABLE_NAME, '表', V_INDEX_NAME, '索引已存在，请勿重复创建') err;
        ELSE -- 索引不存在则创建索引
			IF V_INDEX_TYPE = 0 THEN
				-- 创建普通索引
				SET @V_SQL = CONCAT(" ALTER TABLE ", V_TABLE_SCHEMA, ".", V_TABLE_NAME, " ADD INDEX ", V_INDEX_NAME, "(", V_INDEX_COL, ")");
				PREPARE sqlStmt FROM @V_SQL;
				EXECUTE sqlStmt;
            ELSEIF V_INDEX_TYPE = 1 THEN
				-- 判断是否有重复数据
				SET @V_SQL = CONCAT(" SELECT MAX(b.con) con INTO @V_NON_UNIQUE FROM (SELECT COUNT(1) con FROM ", V_TABLE_SCHEMA, ".", V_TABLE_NAME, " GROUP BY ", V_INDEX_COL, " HAVING COUNT(1) > 1) b");
				PREPARE sqlStmt FROM @V_SQL;
				EXECUTE sqlStmt;
				DEALLOCATE PREPARE sqlStmt;
				set V_NON_UNIQUE = @V_NON_UNIQUE;
				IF V_NON_UNIQUE > 0 THEN
					SELECT CONCAT(V_TABLE_SCHEMA, ".", V_TABLE_NAME, '表', V_INDEX_COL, '列，存在重复数据，无法创建唯一索引') err;
                ELSE
					-- 创建唯一索引
					SET @V_SQL = CONCAT(" ALTER TABLE ", V_TABLE_SCHEMA, ".", V_TABLE_NAME, " ADD UNIQUE INDEX ", V_INDEX_NAME, "(", V_INDEX_COL, ")");
					PREPARE sqlStmt FROM @V_SQL;
					EXECUTE sqlStmt;
                END IF;
            END IF;
